...
import { templateStrings } from './i18n';
...
.invoice-doc .header { display: flex; justify-content: space-between; align-items: center; gap: 24px; }
...
.invoice-doc .num { font-size: 18px; font-weight: 600; color: var(--accent); }
...
.invoice-doc .signatures { margin-top: 56px; display: flex; align-items: flex-end; gap: 40px; }
...
export function ClassicTemplate({ doc }: { doc: InvoiceDocument }) {
  ...
  const notesText = doc.notes?.trim() || t.defaultTerms;
  const renderedNotes = renderNotes(notesText, {
    ...
    <div className="invoice-doc">
      ...
      <div className="totals">
        ...
        <div className="row grand">
          <span>{t.total}</span>
          <span className="num-col">
            {formatMoney(doc.total, currency, locale)}
          </span>
        </div>
      ...
      <div className="signatures">
        <div className="sign">
          <div className="sigline">
            {t.sender} {doc.sender?.name && `(${doc.sender?.name})`}
          </div>
        </div>
        <div className="sign">
          ...
          <div className="sigline">
            {t.receiver} {doc.receiver?.name && `(${doc.receiver?.name})`}
          </div>
        </div>
      </div>
    </div>
